...
class JointEpisodeDataset(Dataset):
            ...
            for ep_id in valid_eps:
                ...
                for seq_idx, frame_key in enumerate(valid_keys):
                    ...
                    self.frames.append({
                        ...
                        "nai_text":     nai_text,      # per-frame NAI or episode instruction
                        "K":            K,
                        "raw_costs":    raw_costs,     # [K] float64
                        "cat_names":    cat_names,     # [K] str
                        "target_k":     target_k if target_k is not None else -1,
                        "target_cat":   target_cat,    # category of the selected anchor object
                        "nai_matched":  nai_matched,   # False → OGCL disabled for this frame
                        "nai_is_global_min": nai_is_global_min,  # False → cheaper non-NAI object exists → OGCL off
                        "action_label": action_label,  # [T, 4] float32
                        "wp_mask":      wp_mask,       # [T] — 0 when no target or clamped
                        "action_mask":  float(action_mask),
                        "dist_label":   dist_label_norm,  # float [0, 1]: physical dist/25m
                    })
                    n_frames += 1
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # ---- NAI text → tokens -----------------------------------------
        # nai_text resolved at index time: per-frame NAI or episode instruction fallback.
        nai_text = meta["nai_text"]
        ...
        target_k_idx      = meta.get("target_k", -1)
        class_match = np.zeros(K, dtype=bool)
        if target_cat and nai_matched and nai_is_global_min and 0 <= target_k_idx < K:
            class_match[target_k_idx] = True
        ...
        dist_label_norm = meta["dist_label"]            # float [0, 1]: physical dist/25m
        has_target   = False
        target_bearing = 0.0
        ...
        return {
            ...
            "dist_label":     torch.tensor(dist_label_norm, dtype=torch.float32),
            ...
            # Pre-selected target object index (NAI-class min-cost or global min-cost fallback, -1 if none)
...
def joint_collate_fn(batch: list[dict]) -> dict:
    ...
    return {
        ...
        # Per-frame selected target object index [B] int64; -1 means no valid target
...
def create_joint_dataloaders(
    ...
    """Return (train_loader, val_loader) with a frame-level split.

    All episodes are indexed first (so every scene is seen in both splits),
    then frames are split randomly.  This prevents scene-level distribution
    shift that would cause val loss to plateau while train loss drops.
    """
    with h5py.File(h5_path, "r") as hf:
        all_keys = sorted(hf.keys())
    ...
    # Build a single dataset over all episodes, then split its frame index.
    full_ds = JointEpisodeDataset(h5_path, all_keys, **ds_kw)
